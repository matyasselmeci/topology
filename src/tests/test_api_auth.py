...
TEST_TOKEN = b"test-token-abcd"
TEST_TOKEN_B64 = base64.urlsafe_b64encode(TEST_TOKEN).decode()
UNKNOWN_TOKEN = b"not-a-real-token-xyz"
UNKNOWN_TOKEN_B64 = base64.urlsafe_b64encode(UNKNOWN_TOKEN).decode()
...
class TestApiKeyAuth:
    ...
    def test_valid_bearer_token_authorizes(
        ...
        response = client.get(
            "/miscuser/xml", headers={"Authorization": f"Bearer {TEST_TOKEN_B64}"}
        )
    ...
    def test_invalid_non_empty_bearer_returns_401(
        ...
        response = client.get(
            "/miscuser/xml", headers={"Authorization": f"Bearer {UNKNOWN_TOKEN_B64}"}
        )
    ...
    def test_valid_dn_with_bad_bearer_is_unauthorized(
        ...
        response = client.get(
            "/miscuser/xml",
            headers={"Authorization": f"Bearer {UNKNOWN_TOKEN_B64}"},
    ...
    def test_contacts_dn_and_bearer_expose_same_fields(
        ...
        resp_bearer = client.get(
            "/contacts", headers={"Authorization": f"Bearer {TEST_TOKEN_B64}"}
        )
    ...
    def test_oasis_managers_dn_and_bearer_both_200(
        ...
        resp_bearer = client.get(
            ...
            headers={"Authorization": f"Bearer {TEST_TOKEN_B64}"},
        )
    ...
    def test_vosummary_xml_dn_and_bearer_expose_same_fields(
        ...
        resp_bearer = client.get(
            "/vosummary/xml", headers={"Authorization": f"Bearer {TEST_TOKEN_B64}"}
        )
    ...
    def test_successful_bearer_logs_hash_prefix_and_owner(
        ...
        assert TEST_TOKEN.decode() not in caplog.text
    ...
    def test_rejected_bearer_logs_hash_prefix_only(
        ...
        assert token_hash[7:15] in caplog.text
        assert UNKNOWN_TOKEN.decode() not in caplog.text
